    ...
    <div class="container-fluid px-4">
        ...
        <div class="row justify-content-center">
            <div class="col-md-10">
                ...
                <div class="calculation-card">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>Garantía Depositada:</strong> ₲ {{ number_format($garantiaOriginal, 0, ',', '.') }}
                            </p>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="date-comparison">
                            ...
                            @if ($diasRetraso > 0)
                                <div class="alert alert-warning mt-3 mb-0">
                                    ...
                                    <strong>Retraso detectado:</strong> El cliente se retrasó {{ $diasRetraso }}
                                    día{{ $diasRetraso > 1 ? 's' : '' }} en la devolución.
                                </div>
                            @else
                    ...
                    <div class="card-body">
                        ...
                        @if ($diasRetraso > 0)
                            <div class="alert alert-info">
                                <strong>Cálculo:</strong> {{ $diasRetraso }} día{{ $diasRetraso > 1 ? 's' : '' }} × ₲
                                {{ number_format($multaDiaria, 0, ',', '.') }} = ₲
                                {{ number_format($multaTotal, 0, ',', '.') }}
                            </div>
                        @endif
                    </div>
                ...
                <div
                    class="card mb-4 {{ $montoDevolver > 0 ? 'result-card' : ($montoDevolver == 0 ? 'warning-card' : 'danger-card') }}">
                        ...
                        @if ($montoDevolver <= 0)
                            <div class="alert alert-warning mt-3">
                                ...
                                @if ($multaTotal > $garantiaOriginal)
                                    El cliente debe pagar un adicional de ₲
                                    {{ number_format($multaTotal - $garantiaOriginal, 0, ',', '.') }}.
                                @else
                ...
                {{-- Reemplazar el formulario existente en multas.blade.php --}}
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('devoluciones.procesar', $alquiler->id) }}" method="POST"
                            id="devolucionForm">
                            @csrf

                            <!-- NUEVO: Sección de ajustes manuales -->
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    <div class="card border-warning">
                                        <div class="card-header bg-warning text-dark">
                                            <h6 class="mb-0"><i class="fas fa-edit me-2"></i>Ajustes Manuales (Opcional)
                                            </h6>
                                        </div>
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <label for="multa_aplicada_real" class="form-label">Multa a Aplicar
                                                        Realmente</label>
                                                    <div class="input-group">
                                                        <span class="input-group-text">₲</span>
                                                        <input type="number" class="form-control" id="multa_aplicada_real"
                                                            name="multa_aplicada_real" value="{{ $multaTotal }}"
                                                            min="0" step="1000">
                                                    </div>
                                                    <small class="text-muted">Multa calculada: ₲
                                                        {{ number_format($multaTotal, 0, ',', '.') }}</small>
                                                </div>
                                                <div class="col-md-6">
                                                    <label for="monto_devuelto_real" class="form-label">Monto a Devolver
                                                        Realmente</label>
                                                    <div class="input-group">
                                                        <span class="input-group-text">₲</span>
                                                        <input type="number" class="form-control" id="monto_devuelto_real"
                                                            name="monto_devuelto_real" value="{{ $montoDevolver }}"
                                                            min="0" step="1000">
                                                    </div>
                                                    <small class="text-muted">Monto calculado: ₲
                                                        {{ number_format($montoDevolver, 0, ',', '.') }}</small>
                                                </div>
                                            </div>
                                            <div class="row mt-3">
                                                <div class="col-md-12">
                                                    <label for="motivo_ajuste" class="form-label">Motivo del
                                                        Ajuste</label>
                                                    <select class="form-select" id="motivo_ajuste" name="motivo_ajuste">
                                                        <option value="">Sin ajuste</option>
                                                        <option value="cortesia_cliente">Cortesía al cliente</option>
                            ...
                            <!-- Campos ocultos para los valores calculados -->
                            <input type="hidden" name="multa_calculada" value="{{ $multaTotal }}">
                            <input type="hidden" name="monto_devuelto_calculado" value="{{ $montoDevolver }}">
...
@push('js')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('devolucionForm').addEventListener('submit', function(e) {
                ...
                if (diasRetraso === 0) {
                    ...
                    detalles =
                        `Retraso de ${diasRetraso} día${diasRetraso > 1 ? 's' : ''}. Se devolverá ₲ ${montoDevolver.toLocaleString()} al cliente.`;
                } else if (montoDevolver === 0) {
                    detalles =
                        `Retraso de ${diasRetraso} día${diasRetraso > 1 ? 's' : ''}. No se devolverá dinero (multa igual a garantía).`;
                } else {
                    const adicional = multaTotal - garantiaOriginal;
                    detalles =
                        `Retraso de ${diasRetraso} día${diasRetraso > 1 ? 's' : ''}. El cliente debe pagar un adicional de ₲ ${adicional.toLocaleString()}.`;
                }
    ...
    <!-- Agregar JavaScript para cálculo automático -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const multaInput = document.getElementById('multa_aplicada_real');
            const montoDevueltoInput = document.getElementById('monto_devuelto_real');
            const garantiaOriginal = {{ $garantiaOriginal }};

            // Recalcular monto a devolver cuando cambia la multa
            multaInput.addEventListener('input', function() {
                const multaAplicada = parseFloat(this.value) || 0;
                const nuevoMontoDevolver = Math.max(0, garantiaOriginal - multaAplicada);
                montoDevueltoInput.value = nuevoMontoDevolver;
            });

            // Recalcular multa cuando cambia el monto a devolver
            montoDevueltoInput.addEventListener('input', function() {
                const montoDevolver = parseFloat(this.value) || 0;
                const nuevaMulta = Math.max(0, garantiaOriginal - montoDevolver);
                multaInput.value = nuevaMulta;
            });
        });
    </script>
@endpush
