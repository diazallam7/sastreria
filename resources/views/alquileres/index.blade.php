{{-- Modificar: resources/views/alquileres/index.blade.php --}}
@extends('template')
                                    ...
                                    <ul class="list-unstyled mb-0">
                                        @foreach ($alquiler->stockItems as $prenda)
                                            @php
                                                ...
                                                $talleName = 'N/A';
                                                if ($talle) {
                                                    $talleName = $talle->talle;
                                                } else {
                                                    // Intentar obtener el talle directamente de la base de datos
                                                    $talleDirecto = DB::table('alquiler_stock')
                                                        ->join(
                                                            'talle_stock',
                                                            'alquiler_stock.talle_id',
                                                            '=',
                                                            'talle_stock.id',
                                                        )
                                                        ->where('alquiler_stock.alquiler_id', $alquiler->id)
                                                        ->where('alquiler_stock.stock_id', $prenda->id)
                                                        ->select('talle_stock.talle')
                                                        ->first();

                                                    if ($talleDirecto) {
                                                        $talleName = $talleDirecto->talle;
                                                    }
                                                }
                                            @endphp
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <ul>
                                @foreach ($alquiler->stockItems as $prenda)
                                    @php
                                        $talle = \App\Models\TalleStock::find($prenda->pivot->talle_id);
                        ...
                        <div class="modal-footer">
                            ...
                            <form action="{{ route('alquileres.alquileres.devolver', ['alquiler' => $alquiler->id]) }}"
                                method="POST">
                                @csrf
                                @method('PATCH')
