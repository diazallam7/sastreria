{{-- Archivo: resources/views/compras/edit.blade.php --}}
@extends('template')
            ...
            <form action="{{ route('compras.update', $compra->id) }}" method="POST" id="compraForm">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('nombre_producto') is-invalid @enderror" 
                               name="nombre_producto" id="nombre_producto" value="{{ old('nombre_producto', $compra->nombre_producto) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('fecha_compra') is-invalid @enderror" 
                               name="fecha_compra" id="fecha_compra" value="{{ old('fecha_compra', $compra->fecha_compra->format('Y-m-d')) }}" required>
                        @error('fecha_compra')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('precio_compra') is-invalid @enderror" 
                               name="precio_compra" id="precio_compra" value="{{ old('precio_compra', $compra->precio_compra) }}" required>
                        @error('precio_compra')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('precio_venta') is-invalid @enderror" 
                               name="precio_venta" id="precio_venta" value="{{ old('precio_venta', $compra->precio_venta) }}" required>
                        @error('precio_venta')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('observacion') is-invalid @enderror" 
                              name="observacion" id="observacion" rows="3">{{ old('observacion', $compra->observacion) }}</textarea>
                    @error('observacion')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div id="tallesContainer">
                        @foreach($compra->talles as $index => $talle)
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        let talleIndex = {{ count($compra->talles) }};
